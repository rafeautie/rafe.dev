...
import type { UseGameEventHandlerProps } from '@/race/client/hooks/useGameEvent'
...
export const ConfettiLayer = ({ className }: ConfettiLayerProps) => {
  ...
  const popConfettiForWinner: UseGameEventHandlerProps['challengeResolution'] =
    (challengeResolution) => {
      ...
      setTimeout(
        () => {
          ...
          if (
            [
              challengeResolution.winnerId,
              challengeResolution.loserId,
            ].includes(ownPlayerCar?.id ?? '')
          ) {
            soundEffects.play(
