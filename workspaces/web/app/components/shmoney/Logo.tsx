...
const LogoGradient = lazy(() => import('~/components/shmoney/LogoGradient'));

// The MoneyBag02 icon from @hugeicons/core-free-icons, inlined so the page
// does not need the hugeicons packages.
...
export function Logo({ className }: { className?: string }) {
	return (
		<div
			className={cn(
				'relative flex aspect-square size-8 shrink-0 items-center justify-center overflow-hidden rounded-lg text-white',
				className
			)}
		>
			<ClientOnly>
			...
			<MoneyBagIcon className="relative h-1/2 w-1/2" />
		</div>
	);
}
